...
export const requestLeave = async (req: AuthRequest, res: Response): Promise<void> => {
	try {
		...
		if (!reason || !startDate || (!(days || endDate))) {
			res.status(400).json({ message: 'Missing required fields: reason, startDate, and (days or endDate)' });
			return;
		}
...
export const leaveApproval = async (req: AuthRequest, res: Response): Promise<void> => {
	try {
		...
		const leave = await LeaveRequest.findOne({ where: { id }, include: [User] });
		if (!leave) {
		...
		if (approverRole !== 'admin' && !isManager) {
			res.status(403).json({ message: 'Forbidden: only manager or admin can review leave requests' });
			return;
		}
...
export const calendarView = async (req: AuthRequest, res: Response): Promise<void> => {
	try {
		...
		const leaves = await LeaveRequest.findAll({ where, include: [{ model: User, attributes: ['id', 'fullName', 'role'] }] });
		res.status(200).json({ month: `${year}-${String(month).padStart(2, '0')}`, data: leaves });
	} catch (error) {
...
export const editRequestLeave = async (req: AuthRequest, res: Response): Promise<void> => {
	try {
		const requesterRole = req.user?.role;
		if (requesterRole !== 'admin') {
			res.status(403).json({ message: 'Forbidden: only admin can edit leave requests' });
			return;
		}
		...
		if (endDate !== undefined) {
			...
			const compareStart = startDate !== undefined ? new Date(startDate) : leave.startDate;
			if (e < compareStart) {
				res.status(400).json({ message: 'endDate cannot be before startDate' });
				return;
			}
