...
jest.mock('../controllers/reimbursementController', () => ({
  createReimbursement: jest.fn((req, res) =>
    res.status(201).json({
      id: 10,
      type: 'reimbursement',
  ...
  getReimbursementRequests: jest.fn((req, res) =>
    res.status(200).json([
...
jest.mock('../controllers/payrollController', () => ({
  addManualAdjustment: jest.fn((req, res) =>
    res.status(201).json({
      id: 99,
      type: req.body.type,
      amount: req.body.amount,
    ...
    return res.status(200).json([
      {
        userId: 1,
        baseSalary: base,
        totalAdjustments: additions - deductions,
        netSalary: base + additions - deductions,
      },
    ]);
  }),
  getSalarySlip: jest.fn((req, res) =>
    res.status(200).json({
      userId: 1,
  ...
  updatePayrollStatus: jest.fn((req, res) =>
    res
      .status(200)
      .json({ id: parseInt(req.params.id), status: req.body.status }),
  ),
}));
...
describe('Financial Modules Routes Validation', () => {
  ...
  describe('Reimbursement Routes', () => {
    it('creates reimbursement requested dynamically (POST /api/reimbursement)', async () => {
      const res = await request(app)
        ...
        .send({ title: 'Travel', amount: 50000 });
    ...
    it('rejects an approval review without reason mathematically guarding the workflow (PUT /api/reimbursement/:id/review)', async () => {
      const res = await request(app)
        .put('/api/reimbursement/10/review')
    ...
    it('fetches a filtered list of reimbursement requests correctly (GET /api/reimbursement/requests)', async () => {
      const res = await request(app).get(
        '/api/reimbursement/requests?status=pending',
      );
      expect(res.status).toBe(200);
  ...
  describe('Payroll Routes', () => {
    it('handles generic incentive inputs explicitly (POST /api/payroll/adjustments)', async () => {
      const res = await request(app).post('/api/payroll/adjustments').send({
        userId: 1,
        title: 'Bonus Tahunan',
        type: 'bonus',
        amount: 1000000,
